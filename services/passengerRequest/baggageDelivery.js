...
// Факт услуги «Доставка багажа»: сколько пассажиров перечислено во ВСЕХ поездках.
// Услуга одна на все поездки, поэтому статус пересчитывается по этой сумме.
//
// Функция намеренно НЕ переиспользует transferFactCount из serviceStatus.js и не
// сводится с ним в одну. У трансфера факт поездки = max(длина списка,
// transportedCount): там диспетчер вводит «перевезено N» без поимённого списка.
// У багажа transportedCount не заполняется вовсе — его поимённый список и есть
// факт. Разница наблюдаемая: подставь сюда трансферную формулу — багаж начнёт
// реагировать на чужое поле, которого в его сценариях не бывает.
export const countTripPeople = (drivers) => {
  if (!Array.isArray(drivers)) return 0
  return drivers.reduce(
    (sum, driver) =>
      sum + (Array.isArray(driver?.people) ? driver.people.length : 0),
    0
  )
}
