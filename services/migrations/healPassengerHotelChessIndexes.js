  ...
  for (const request of requests) {
    ...
    const nextHotels = hotels.map((hotel, hotelIdx) => {
      ...
      const nextPeople = (hotel?.people || []).map((person) => {
        ...
        // Гостей без размещения не чиним: у скрипта нет даты, которой можно
        // открыть интервал — операции, давшей бы её, не было. Такая форма
        // залечивается сама следующей мутацией проживания, и с закрытием
        // дефекта №21 та мутация ставит дату операции, а не время записи.
        if (chesses.length === 0) {
          withoutChesses += 1
          return person
        }
