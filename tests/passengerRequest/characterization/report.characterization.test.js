...
test("ДЕФЕКТ №2: в лог сохранения отчёта уходит запись ОТЧЁТА, а не заявка", async () => {
  ...
  // Второй поиск заявки — из почтовой ветки, и ищет он по идентификатору отчёта.
  assert.deepEqual(run.requestLookups, ["req-1", run.result.id])
})
...
test("ДЕФЕКТ №3: сохранение отчёта зовёт лог БЕЗ skipEmail — почтовая ветка входит на каждом автосейве", async () => {
  // Автосохранение отчёта дёргается флашем на размонтировании страницы и перед
  // выгрузкой Excel, то есть многократно. skipEmail не передан, поэтому каждое
  // сохранение доходит до сборки и отправки письма; единственный ограничитель —
  // дедуп в памяти процесса (services/notification/notificationRateGuard.js,
  // TTL 90 с, ключ включает entityId), который живёт НИЖЕ по стеку и на вход в
  // ветку не влияет. Реестр дефектов спеки, №3.
  const first = await runReport(
    "savePassengerRequestHotelReport",
    saveArgs([{ fullName: "Иванов Иван" }])
  )
  ...
  // Вход в почтовую ветку виден по второму поиску заявки (его делает
  // resolveCreatorAirlineDepartment внутри sendRequestPartyEmail).
  assert.equal(first.requestLookups.length, 2, "первое сохранение идёт в почту")
  assert.equal(second.requestLookups.length, 2, "и второе тоже")

  // Маршрут письма для этого слага — обычное «обновление заявки»: отдельного
  // почтового действия у отчёта нет.
  assert.equal(
