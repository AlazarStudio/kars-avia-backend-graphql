...
test("маршрутизация писем: три из восьми действий группы уходят как обычное обновление", async () => {
  // Письма через двойник не видны (почтовая ветка в try/catch), поэтому
  // маршрутизацию закрепляем на чистой функции. Пакетное добавление и
  // присвоение номера комнаты в набор «шахматки гостиниц» не входят, хотя по
  // смыслу это то же заселение.
  const chess = [
