...
test("маршрутизация писем: одно из восьми действий группы уходит как обычное обновление", async () => {
  ...
  // получал вовсе. Теперь входит наравне с одиночным. Присвоение номера комнаты
  // тоже переведено в набор: по смыслу это изменение размещения, и письмо о нём
  // должно фильтроваться тем же флагом меню. А правка карточки гостиницы в набор
  // по-прежнему не входит, и это осознанно: там меняется сама гостиница, а не
  // чьё-то размещение, и шаблон «изменение размещения» ей не подходит.
  const chess = [
  ...
  const generic = ["update_passenger_request_hotel"]
